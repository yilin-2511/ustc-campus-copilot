...
你由 CloseClaw 团队（张义临、钟华杰）开发，参加"一〇七杯"智能体赛道比赛。
...
工具3: search_course_info
  什么时候用：
  - 用户想查某门课程的基本信息（学分、考核方式、教材、开课院系）
  - 用户想查考试安排或空教室（返回教务系统链接）
  - 注意：不提供上课时间/地点（需自行在教务系统查询），不提供教师评价（用 search_forum_knowledge）
...
3. 经验+平台都要 → 同时调用 search_forum_knowledge 和 navigate_to_platform，
   两个工具互不依赖，一次返回节省时间，然后合成回答。
...
- **严格禁止从训练数据补充事实。** 涉及以下内容时，只能用工具返回的数据，不确定就别说：
  具体地点/楼层/门牌号、网址/联系方式、政策条款/截止日期、价格/费用。
  比如工具返回"1958咖啡馆"，但没说在哪个校区，你就不能说"中区1958咖啡"。
- **工具返回的具体信息必须展示。** 比如 navigate_to_platform 返回了群号、网址，
  就要在回答中列出，不能说"可以去第二课堂找社团"而不给群号。
- 如果工具返回了数据，回答末尾标注数据来源（论坛经验/官方平台/新生指南/QQ群）。
...
TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "查询科大教务系统公开数据。支持三种查询："
                "course_schedule=课程搜索(学分/考核方式/教材/院系，注意不包含上课时间和地点)、"
        ...
        if msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                try:
                    ...
                    func_args = {}
                tasks.append((tc, func_name, func_args))

            # 并行执行所有工具（最多 3 个，用线程池）
            results = {}
            with ThreadPoolExecutor(max_workers=len(tasks)) as pool:
                futures = {}
                for tc, func_name, func_args in tasks:
                    if on_tool_start:
                        try:
                    ...
                    def _run(h=handler, fa=func_args, fn=func_name):
                        if h:
                            try:
                                return h(**fa)
                            except Exception as e:
                                return "[工具执行错误] {}: {}".format(fn, e)
                        return "[未知工具] {}".format(fn)

                    futures[pool.submit(_run)] = (tc, func_name)

                for future in as_completed(futures):
                    tc, func_name = futures[future]
                    result = future.result()
                    results[tc.id] = (func_name, result)

            # 按原始顺序追加 tool 结果
            for tc, func_name, func_args in tasks:
                func_name, result = results[tc.id]
                if on_tool_end:
                    try:
